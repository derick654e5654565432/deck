...
const clampMin = (v, def) => { const n = Number(v); return Number.isFinite(n) ? Math.max(0, Math.min(1440, Math.round(n))) : def; };
const clampDur = (v, def) => { const n = Number(v); return Number.isFinite(n) ? Math.max(15, Math.min(720, Math.round(n))) : def; };

// Enrich a raw plan_blocks row with the project/todo card data the board needs.
function enrichPlan(b) {
  const out = { id: b.id, kind: b.kind, ref_id: b.ref_id, day: b.day, start_min: b.start_min, duration_min: b.duration_min, done: !!b.done };
  if (b.kind === 'todo') {
...
app.get('/api/plan', requireAuth, (req, res) => {
  ...
  const rows = (from && to)
    ? db.prepare('SELECT * FROM plan_blocks WHERE owner_id = ? AND day BETWEEN ? AND ? ORDER BY day, start_min, id').all(req.user.id, from, to)
    : db.prepare('SELECT * FROM plan_blocks WHERE owner_id = ? ORDER BY day, start_min, id').all(req.user.id);
  res.json({ blocks: rows.map(enrichPlan).filter(Boolean) });
});

app.post('/api/plan', requireAuth, (req, res) => {
  let { kind, ref_id, day, start_min, duration_min } = req.body || {};
  ...
  const now = Date.now();
  const r = db.prepare('INSERT INTO plan_blocks (owner_id, kind, ref_id, day, start_min, duration_min, done, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?)')
    .run(req.user.id, kind, ref_id, day, clampMin(start_min, 540), clampDur(duration_min, 60), now, now);
  res.json({ id: Number(r.lastInsertRowid) });
});
...
app.put('/api/plan/:id', requireAuth, (req, res) => {
  ...
  const start_min = req.body.start_min === undefined ? cur.start_min : clampMin(req.body.start_min, cur.start_min);
  const duration_min = req.body.duration_min === undefined ? cur.duration_min : clampDur(req.body.duration_min, cur.duration_min);
  ...
  db.prepare('UPDATE plan_blocks SET day = ?, start_min = ?, duration_min = ?, done = ?, updated_at = ? WHERE id = ?')
    .run(day, start_min, duration_min, done, Date.now(), cur.id);
  res.json({ ok: true });
});
